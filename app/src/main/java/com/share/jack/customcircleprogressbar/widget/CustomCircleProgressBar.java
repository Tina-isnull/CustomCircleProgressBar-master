...
public class CustomCircleProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        double x1 = outsideRadius + outsideRadius * Math.sin(angle * PI / 180);
        double y1 = outsideRadius + outsideRadius *-1* Math.cos(angle * PI / 180);
        paint.setColor(Color.BLACK);
        canvas.drawCircle((float) Math.abs(x1), (float) Math.abs(y1), 2, paint);
